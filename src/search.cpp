...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1642 - int(delta) * 1024 / int(rootDelta)) / 1024 + (!i && r > 916);
        ...
        return (r + 1642 - int(delta) * 1024) / 1024 + (!i && r > 916);
  }
  ...
  int stat_bonus(Depth d) {
    return std::min((12 * d + 282) * d - 349 , 1400);
  }
...
inline uint8_t getInitialShashinValue(Position& pos) {
  if (!highTal && !middleTal && !lowTal && !capablanca && !highPetrosian && !middlePetrosian && !lowPetrosian)
    return SHASHIN_POSITION_TAL_CAPABLANCA_PETROSIAN;    
  if (lowTal && capablanca && !highPetrosian && !middlePetrosian && !lowPetrosian)
    return SHASHIN_POSITION_TAL_CAPABLANCA;
  if ((highTal || middleTal ||lowTal)&& !capablanca && !highPetrosian && !middlePetrosian && !lowPetrosian)
    return SHASHIN_POSITION_TAL;
  if (!highTal && !middleTal && !lowTal && capablanca && !highPetrosian && !middlePetrosian && !lowPetrosian)
    return SHASHIN_POSITION_CAPABLANCA;
  if (!highTal && !middleTal && !lowTal && capablanca && lowPetrosian)
    return SHASHIN_POSITION_CAPABLANCA_PETROSIAN;
  if (!highTal && !middleTal && !lowTal && !capablanca && (highPetrosian || middlePetrosian || lowPetrosian))
    return SHASHIN_POSITION_PETROSIAN;
...
void Thread::search() {
  ...
  iccf = Options["Iccf_MultiPV"];
  if (!iccf) multiPV = size_t(Options["MultiPV"]);
  multiPV = iccf && iccf_flag ?  size_t(pow(2, iccf)) : 1;
  ...
  lmrAverage.set(6, 100);
  optimism[us] = optimism[~us] = VALUE_ZERO;

  int searchAgainCounter = 0;
  int stabilityCount = 0;

  int failedAspirationsCurrentID = 0;
  ...
  if (
		(
			...
			(
             ...
             ||
             (rootPos.this_thread()->shashinValue == SHASHIN_POSITION_TAL_CAPABLANCA)
             ||
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
	...
	for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
	{
	    ...
	    if (rootDepth >= 4)
	    {
			...
			delta = Value(6 + 4 * failedAspirationsPrevID)+ int(prev) * prev / 15620;//aspiration patch 
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    bool givesCheck, improving, didLMR, priorCapture, singularQuietLMR, isMate;
    bool capture, captureOrPromotion, doFullDepthSearch, moveCountPruning, ttCapture, kingDanger, ourMove;
    Piece movedPiece;
    int moveCount, captureCount, quietCount, improvement=0, rootDepth, complexity=0;
    ...
    if (    PvNode
        && !ttMove)
        depth -= 3 +((pos.this_thread()->shashinQuiescentCapablancaMaxScore)?0: (- (!ss->ttHit && complexity > 1000)));
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
              ...
              &&  tte->depth() >= depth - 3)
          {
              ...
              Depth singularDepth = (depth - 1 + (!ss->inCheck && eval - ss->staticEval < -250)) / 2;
              ...
              // If the eval of ttMove is greater than beta, we reduce it (negative extension)
                  ...
                  extension = std::max(-4, -(newDepth - 3)), negExt = true;
            ...
            || (bestValue >= VALUE_TB_WIN_IN_MAX_PLY)))
      {
		  ...
		  thisThread->lmrAverage.update(value > alpha);
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    bool pvHit, givesCheck, capture, gameCycle=false;
    int moveCount;
...
// ttValue can be used as a better position evaluation (~7 Elo)
	  ...
	  {
	      bestValue = ttValue;
	      updateShashinValues(pos,ttValue);
	  }
