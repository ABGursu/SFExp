...
#define USE_CUSTOM_HASHER
    ...
    #include "sparsehash/dense_hash_map"
        ...
        template<typename tKey, typename tVal> class StockfishMZMap : public google::dense_hash_map<tKey, tVal, KeyHasher, KeyHasher>
        {
        public:
            StockfishMZMap(tKey emptyKey, tKey deletedKey)
            {
    ...
    #else
        template<typename tKey, typename tVal> class StockfishMZMap : public google::dense_hash_map<tKey, tVal>
        {
        public:
            StockfishMZMap(tKey emptyKey, tKey deletedKey)
            {
    ...
    template<typename tVal> class StockfishMZKeyMap : public StockfishMZMap<Stockfish::Key, tVal>
    {
    public:
        StockfishMZKeyMap() : StockfishMZMap<Key, tVal>((Key)0, (Key)-1)
        {
        }
    };
#else
    #include <unordered_map>
    template<typename tKey, typename tVal> using StockfishMZMap = unordered_map<tKey, tVal>;
    template<typename tVal> using StockfishMZKeyMap = StockfishMZMap<Key, tVal>;
#endif
...
namespace Experience
{
    ...
    // Typedefs
    ////////////////////////////////////////////////////////////////
    typedef StockfishMZKeyMap<ExpEntryEx*> ExpMap;
    typedef StockfishMZKeyMap<ExpEntryEx*>::iterator ExpIterator;
    typedef StockfishMZKeyMap<ExpEntryEx*>::const_iterator ExpConstIterator;
    ...
    //Note:    'filename' is optional. If omitted, then the default experience filename (StockfishMZ.exp) will be used
