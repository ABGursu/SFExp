...
#undef S
  ...
  Score Evaluation<T>::pieces() {
    ...
    while (b1)
    {
        ...
        else if (((Pt == BISHOP) && (attacks_bb<BISHOP>(s, pos.pieces(PAWN)) & kingRing[Them])&& (pos.this_thread()->shashinValue != SHASHIN_POSITION_CAPABLANCA)))
...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  else
  {
      ...
      optimism = (pos.this_thread()->shashinValue==SHASHIN_POSITION_CAPABLANCA)? 
                    (Value)0: (optimism * (269 + nnueComplexity) / 256);
